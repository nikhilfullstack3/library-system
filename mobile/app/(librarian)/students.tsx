...
function getLiveTimer(student: any, now: number) {
  if (student.fullDay || !student.currentlyInLibrary || !student.activeSessionStartedAt) return null;
  const elapsedMs = Math.max(0, now - new Date(student.activeSessionStartedAt).getTime());
...
function getShiftWarning(student: any, now: number) {
...
// Timer lives inside the card — only cards with live sessions tick
const LiveStudentCard = memo(function LiveStudentCard({ student, onPress }: { student: any; onPress: () => void }) {
  ...
  const liveTimer = getLiveTimer(student, now);
  const shiftWarning = getShiftWarning(student, now);
      ...
      <Card>
        <View style={styles.row}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.meta}>Seat {student.seatNumber} • {student.phone}</Text>
          </View>
...
// Static card for students not currently in library — no timer, never re-renders
const StaticStudentCard = memo(function StaticStudentCard({ student, onPress }: { student: any; onPress: () => void }) {
  return (
    <Pressable onPress={onPress}>
      <Card>
        <View style={styles.row}>
        ...
        <Text style={styles.detail}>
          Shift Timing: {student.shiftTiming || student.shift || "-"}
        </Text>
        <Text style={styles.detail}>Login ID: {student.loginId || "-"}</Text>
        <Text style={styles.detail}>Password: {student.issuedPassword || "Issued after payment"}</Text>
      </Card>
  ...
  return (
    <Screen>
      <FlatList
        ...
        renderItem={({ item }) => {
          const onPress = () => router.push(`/(librarian)/student/${item.id}` as never);
          return item.currentlyInLibrary
            ? <LiveStudentCard student={item} onPress={onPress} />
            : <StaticStudentCard student={item} onPress={onPress} />;
        }}
